...
public class TweenModifier
{
    ...
    public IEnumerator Animate(Vector3 offset, float speed, bool reset)
    {
        Vector3 endValue = _startValue + offset;

        float t = 0;

        while (_speedCurve == null)
        {
            yield return null;
        }
        ...
        while (true)
        {
            SetTargetModifier(Vector3.LerpUnclamped(_startValue, endValue, _speedCurve.Evaluate(t)));

            t += Time.deltaTime * speed;
            if (t > 1)
            {
                SetTargetModifier(Vector3.LerpUnclamped(_startValue, endValue, 1));
                _onEnd.Invoke();
                break;
            }
            yield return null;
